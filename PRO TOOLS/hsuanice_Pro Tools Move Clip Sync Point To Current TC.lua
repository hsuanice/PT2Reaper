...
--   project start (timeline 0). All items in scope shift by the same
--   delta so relative positions and individual sync points are preserved.
--
--   REAPER's per-item sync point lives in D_SNAPOFFSET (offset from item
--   start to sync point, in seconds). Sync point absolute position =
--   item.pos + D_SNAPOFFSET.
--
--   Unlike Move Clip Start / End, sync-point variants do NOT split items
--   on partial overlaps — sync point is a whole-item property.
--
--   Scope priority: Razor > Time selection > Item selection. Items in
--   scope are those overlapping the selection (any overlap counts).
--
--   - Tags: Clips, Editing
-- @changelog
--   0.2.0 [260506.1515] - Real implementation. Group move by delta computed
--                          from earliest item's sync absolute position.
...
-- Find earliest item (by start position) → its sync abs position is the reference.
