-- @description hsuanice_Pro Tools Copy and Move Clip End To Play Position
-- @version 0.2.0 [260506.1500]
-- @author hsuanice
-- @link https://forum.cockos.com/showthread.php?p=2910884#post2910884
-- @about
--   Replicates Pro Tools: **Copy and Move Clip End To Play Position**
--   (Opt+Ctrl+K)
--
--   Like "Move Clip End To Play Position" but originals stay intact —
--   only COPIES of the inside-selection portions are placed at the target
--   so the selection END aligns to the EDIT cursor. Selection (TS / razor)
--   follows the copies. Always clones even when delta = 0 (Spot semantics).
--
--   - Tags: Clips, Editing
-- @changelog
--   0.2.0 [260506.1500] - Real implementation. Mirrors Copy and Move
--                          Clip End To Current TC, but target = edit cursor.
...
if mode == "items" then
  ...
  r.PreventUIRefresh(-1)
  r.UpdateArrange()
  r.Undo_EndBlock("Pro Tools: Copy and Move Clip End To Play Position", -1)
  return
end

local delta = target - sel_e
-- NOTE: do NOT short-circuit when delta == 0.
