-- @description hsuanice_Pro Tools Copy and Move Clip End To Current TC
-- @version 0.2.0 [260506.1500]
-- @author hsuanice
-- @link https://forum.cockos.com/showthread.php?p=2910884#post2910884
-- @about
--   Replicates Pro Tools: **Copy and Move Clip End To Current TC**
--   (Opt+Ctrl+I)
--
--   Like "Move Clip End To Current TC" but originals stay intact — only
--   COPIES of the inside-selection portions are placed at the target so
--   the selection END aligns to project start (timeline 0). Selection
--   (TS / razor) follows the copies.
--
--   - Tags: Clips, Editing
-- @changelog
--   0.2.0 [260506.1500] - Real implementation. Mirrors Copy and Move
--                          Clip Start To Current TC, but alignment
--                          reference is the selection END.
...
if mode == "items" then
  ...
  for i = 0, n-1 do
    ...
    local fin = r.GetMediaItemInfo_Value(it, "D_POSITION") + r.GetMediaItemInfo_Value(it, "D_LENGTH")
    if fin > latest then latest = fin end
    sel_items[#sel_items+1] = it
  end
  local delta = target - latest
  r.Undo_BeginBlock()
  ...
  r.PreventUIRefresh(-1)
  r.UpdateArrange()
  r.Undo_EndBlock("Pro Tools: Copy and Move Clip End To Current TC", -1)
  return
end

local delta = target - sel_e
