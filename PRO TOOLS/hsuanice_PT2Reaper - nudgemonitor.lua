...
local function captureState()
  ...
  if item then
    ...
    for j = 0, numItems - 1 do
      ...
      if it == item then
        if j > 0 then
          ...
          if ov > 0 then
            s.crossfadeLeft = ov
            s.xfLeftFoLen   = reaper.GetMediaItemInfo_Value(prev, "D_FADEOUTLEN")
          end
        end
        if j < numItems - 1 then
          ...
          if ov > 0 then
            s.crossfadeRight    = ov
            s.xfRightPos        = nxtPos
            s.xfRightLen        = reaper.GetMediaItemInfo_Value(nxt, "D_LENGTH")
            s.xfRightFiLen      = reaper.GetMediaItemInfo_Value(nxt, "D_FADEINLEN")
            s.xfRightFoLen      = reaper.GetMediaItemInfo_Value(nxt, "D_FADEOUTLEN")
          end
        end
        break
      end
    ...
    s.xfLeftFoLen = nil; s.xfRightFiLen = nil
    s.xfRightPos = nil; s.xfRightLen = nil; s.xfRightFoLen = nil
  end
...
local FIELD_DEFS = {
  ...
  { key = "crossfadeLeft",     label = "XF left overlap"  },
  { key = "xfLeftFoLen",       label = "XF left fo_len"   },
  { key = "crossfadeRight",    label = "XF right overlap" },
  { key = "xfRightPos",        label = "XF right pos"     },
  { key = "xfRightLen",        label = "XF right len"     },
  { key = "xfRightFiLen",      label = "XF right fi_len"  },
  { key = "xfRightFoLen",      label = "XF right fo_len"  },
...
local function drawSnapshotPanel(title, snap)
  ...
  if snap.itemPos then
    ...
    labelValue("  XF left ov  ", snap.crossfadeLeft  and fmtPos(snap.crossfadeLeft)  or "—")
    labelValue("  XF left fo  ", snap.xfLeftFoLen   and fmtPos(snap.xfLeftFoLen)   or "—")
    labelValue("  XF right ov ", snap.crossfadeRight and fmtPos(snap.crossfadeRight) or "—")
    labelValue("  XF right pos", snap.xfRightPos    and fmtPos(snap.xfRightPos)    or "—")
    labelValue("  XF right len", snap.xfRightLen    and fmtPos(snap.xfRightLen)    or "—")
    labelValue("  XF right fi ", snap.xfRightFiLen  and fmtPos(snap.xfRightFiLen)  or "—")
    labelValue("  XF right fo ", snap.xfRightFoLen  and fmtPos(snap.xfRightFoLen)  or "—")
  ...
  if visible then
    ...
    if reaper.ImGui_CollapsingHeader(ctx, "Live State") then
      ...
      if live.itemPos then
        ...
        labelValue("  XF left ov ", live.crossfadeLeft  and fmtPos(live.crossfadeLeft)  or "—")
        labelValue("  XF left fo ", live.xfLeftFoLen   and fmtPos(live.xfLeftFoLen)   or "—")
        labelValue("  XF right ov", live.crossfadeRight and fmtPos(live.crossfadeRight) or "—")
        labelValue("  XF right pos", live.xfRightPos   and fmtPos(live.xfRightPos)    or "—")
        labelValue("  XF right len", live.xfRightLen   and fmtPos(live.xfRightLen)    or "—")
        labelValue("  XF right fi", live.xfRightFiLen  and fmtPos(live.xfRightFiLen)  or "—")
        labelValue("  XF right fo", live.xfRightFoLen  and fmtPos(live.xfRightFoLen)  or "—")
    ...
    local mlChanged, mlVal = reaper.ImGui_InputText(ctx, "##manuallabel", manualLabelBuf)
    if mlChanged then manualLabelBuf = mlVal end
    ...
    if reaper.ImGui_Button(ctx, "  Capture AFTER  ") then
      ...
      if snapBefore and snapAfter then
        ...
        local lbl = (manualLabelBuf ~= "") and manualLabelBuf or "(manual)"
        table.insert(log, 1, { label = lbl, before = snapBefore, after = snapAfter, diffs = diffs })
        if #log > MAX_LOG then table.remove(log) end
      end
    end
    reaper.ImGui_SameLine(ctx)
    ...
    if snapBefore and snapAfter then
      local curDiffs = computeDiffs(snapBefore, snapAfter)
      local diffLabel = (manualLabelBuf ~= "") and manualLabelBuf or "(manual)"
      reaper.ImGui_TextColored(ctx, COL_HEAD, "Diff: " .. diffLabel)
      reaper.ImGui_SameLine(ctx)
      if reaper.ImGui_Button(ctx, "Copy diff") then
        local lines = {
          "Label: " .. diffLabel,
          "Before @ " .. snapBefore.timestamp,
          "After  @ " .. snapAfter.timestamp,
          string.rep("-", 60),
        }
        if #curDiffs == 0 then
          table.insert(lines, "(no changes)")
        else
          for _, d in ipairs(curDiffs) do
            local bStr = (d.before == nil) and "—" or fmtPos(d.before)
            local aStr = (d.after  == nil) and "—" or fmtPos(d.after)
            table.insert(lines, string.format("%-22s  %s  →  %s", d.label, bStr, aStr))
          end
        end
        reaper.ImGui_SetClipboardText(ctx, table.concat(lines, "\n"))
      end
      drawDiffTable(curDiffs)
    end
