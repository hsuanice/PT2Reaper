...
--   **Move Edit Insertion To Next Edit [Extend Selection]**
--
--   ## Behaviour
...
-- Search from selection right end (or cursor if no selection)
local search_from = has_ts and te or cursor_pos

-- Move cursor to search_from to find next transient/edge
...
if math.abs(new_pos - search_from) > EPS then
  -- Cursor goes to left (anchor), selection = anchor → new_pos
