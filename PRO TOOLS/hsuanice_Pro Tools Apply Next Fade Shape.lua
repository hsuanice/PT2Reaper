...
-- @about
--   Replicates Pro Tools: **Apply Next Fade Shape** (Opt+Ctrl+Right)
--   Cycles all fade shapes (fade-in, fade-out, crossfade) in scope to the
--   NEXT shape in the GUI order:
--     1 Linear → 2 Slight Convex (Eq.P.) → 3 Slight Concave →
--     4 Sharp Convex → 5 Sharp Concave → 6 Slight S-Curve → 7 Sharp S-Curve
--   Stops at 7 (no wrap to 1).
--
...
--   increment from is the first detected fade by priority:
...
-- Load PT_Fades library
local _info = debug.getinfo(1, 'S')
local _dir  = _info.source:match('^@(.*[/\\])') or ''
local F = dofile(_dir .. '../Library/hsuanice_PT_Fades.lua')
if not F then
  r.ShowMessageBox("Could not load Library/hsuanice_PT_Fades.lua", "Apply Next Fade Shape", 0)
  return
end
...
-- Anchor priority: xfade > fadein > fadeout
local anchor
...
r.PreventUIRefresh(-1)
r.UpdateArrange()
r.Undo_EndBlock("Pro Tools: Apply Next Fade Shape (" .. F.SHAPE_NAMES[new_shape] .. ")", -1)
